...
function BarchartQntdFretesMes(props){
    ...
    async function handleQntdFretesMes(){
        validarTempoTOKEN();
        await api.get(`/dashboard/qntd/${props.ano}`)
        .then((response)=>{
            const data = response.data;
            const dataArray = [];
            for(let i=0;i<data.length;i++){
                let mes = data[i]["mes"];
                let quantidade = data[i]["valor"];
                dataArray.push({"mes":verifyMonth(mes),"valor":quantidade });
            }
    ...
    function clearChart(){
        d3.select('#chart-container').selectChildren().remove();
    }
...
export  default BarchartQntdFretesMes;